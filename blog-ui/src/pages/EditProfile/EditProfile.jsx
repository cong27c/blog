...
import { getProfile, updateProfile } from "@/services/profileServiece";
...
const EditProfile = () => {
  ...
  const handleImageChange = async (type, event) => {
    ...
    try {
      // Gọi API upload ngay lập tức
      const res = await uploadFile(file);
      setImagePreviews((prev) => ({ ...prev, [type]: res.url }));
      setFormData((prev) => ({ ...prev, [type]: res.url }));
    } catch (err) {
      setErrors((prev) => ({ ...prev, [type]: "Failed to upload image" }));
    }
  };
  ...
  const uploadImages = async () => {
    ...
    if (imageFiles.avatar) {
      const avatarRes = await uploadFile(imageFiles.avatar);
      console.log(avatarRes);
      uploadedUrls.avatar = avatarRes.url;
    }

    if (imageFiles.coverImage) {
      const coverRes = await uploadFile(imageFiles.coverImage);
      uploadedUrls.coverImage = coverRes.url;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await updateProfile(username, submitData);
